...
def get_browser_info():
    remote_debugging_url = 'http://x.x.x.x:8090/json/version'
    try:
...
def get_page_info():
    remote_debugging_url = 'http://x.x.x.x:8090/json/list'
    try:
        response = requests.get(remote_debugging_url)
        data = json.loads(response.text)
        page_info = []
...
def print_specific_cookies(cookies):
    specific_cookies = ['SID', '__Secure-1PSID', 'OSID', '__Secure-1PSIDTS']
...
if pages:
    ...
    if page_index >= 0 and page_index < len(pages):
